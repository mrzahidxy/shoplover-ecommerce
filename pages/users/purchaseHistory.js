...
const purchaseHistory = () => {
   ...
   const [Orders, setOrders] = useState([
      {
         products: [
            { image: speaker, name: "JBL Party Box On-The-Go Portable Party And Wireless Mic", quantity: 1, price: 1400, color: "" },
            { image: phone, name: "Realme Narzo 50a Prime-4GB RAM/128GB ROM Mobile Smartphone", quantity: 2, price: 5000, color: "" },
            { image: bag, name: "Water Resistant Men Crossbody Sling Bag with USB Port", quantity: 1, price: 1400, color: "" },
         ],
         id: "XXZ1111",
         date: "13 Dec, 2022",
         grand_total: 7800,
      ...
      {
         products: [
            { image: earbuds, title: "Realme Tws Wireless Earbuds", quantity: 1, price: 1400, color: "" },
            { image: earbud2, title: "Realme Dizo Go Pods Tws - Black", quantity: 2, price: 2000, color: "" },
         ],
                     ...
                     <div key={index} className='mb-5 hidden md:flex md:flex-col 2xl:w-268 font-medium bg-listBar text-[#071529] max-h-[720px] overflow-auto shadow-md'>
                        ...
                        <div className='grid grid-cols-2 bg-white items-center'>
...

                              {
                                 orders.products.map((products, index2) => (
                                    <div className='flex flex-row my-6 w' key={index2}>
                                       <Image src={products.image} alt='' width={40} height={10} className="ml-10 h-16 w-16 mr-4 rounded-md border-1" />
                           ...
                           <div className='grid grid-cols-4 text-sm font-light'>
                              ...
                              <u className='text-center hidden lg:flex text-blue-400'><button className='text-blue-400' onClick={() => { router.push(`/users/orderDetails/${orders._id}`) }}>View Details</button></u>
                           </div>
...

                              {
                                 orders.products.map((products, index2) => (
                                    <div className='flex flex-row my-4 h-14 px-4' key={index2}>
                                       <Image src={products.image} alt='' width={40} height={10} className="h-14 w-14 mr-4 rounded-full border-1" />
                                       <div className='flex-col'>
